...
exports.FFmpegCompositor = void 0;
...
class FFmpegCompositor {
    constructor() {
        this.tempDir = path_1.default.join(os_1.default.homedir(), 'ContentForge', 'temp');
     ...
     */
    async renderTemplate(template, outputPath, onProgress) {
        ...
        visualLayers.forEach((layer, index) => {
            if (layer.type === 'image' || layer.type === 'video') {
                ...
                if (layer.properties.filters) {
                    layer.properties.filters.forEach((filter) => {
                        if (filter.type === 'scale') {
                            layerFilters.push(`scale=${filter.params.width}:${filter.params.height}:force_original_aspect_ratio=${filter.params.mode || 'decrease'}`);
                        }
                        else if (filter.type === 'zoompan') {
                            // Ken Burns effect
                            const zp = filter.params;
                            layerFilters.push(`zoompan=z='${zp.zoom}':d=${zp.duration}:x='${zp.x}':y='${zp.y}':s=${zp.s}:fps=${zp.fps}`);
                        }
     ...
     */
    async generatePreview(template, timestamp, outputPath) {
        return new Promise((resolve, reject) => {
            ...
            (0, fluent_ffmpeg_1.default)(visualLayer.properties.source)
                .screenshots({
    ...
    estimateRenderTime(template) {
        ...
        const complexity = layerCount <= 3 ? 'low' :
            layerCount <= 6 ? 'medium' : 'high';
        return { estimatedSeconds, complexity };
    }
}
exports.FFmpegCompositor = FFmpegCompositor;
//# sourceMappingURL=FFmpegCompositor.js.map